...
-(id)initWithFrame:(CGRect)frame {
  if(self = [super initWithFrame:frame]) {
    ...
    self.opaque = YES;
     self.center = (self.placement == kBadgePlacementUpperLeft) ? CGPointMake(self.superview.frame.size.width/4, self.superview.frame.size.height*2/3) : CGPointMake(self.superview.frame.size.width, 0);
  }
  return self;
}
- (void)setBadgeValue:(NSInteger)value {
  if(value != 0 || self.displayWhenZero) {
    ...
    self.center = (self.placement == kBadgePlacementUpperLeft) ? CGPointMake(superviewFrame.size.width/4, superviewFrame.size.height*2/3) : CGPointMake(superviewFrame.size.width, 0);
  } else {
